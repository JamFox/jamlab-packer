...
variable "qemu_agent" {
  type        = bool
  default     = true
  description = "Enables QEMU Agent option for this VM. When enabled, then qemu-guest-agent must be installed on the guest. When disabled, then ssh_host should be used."
}
...
variable "ssh_key" {
  type        = string
  default     = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAILayJ7ZP6Z6IDms4ujnz9nRnAJEIXwyto1SbSwfzjqst jamfox"
...
# -- Kickstart variables -- #
variable "ks_partitioning_commands" {
  type        = list(string)
  default     = [
    "clearpart --all --initlabel",
  ]
  description = "List of partitioning commands to pass to Kickstart."
}
